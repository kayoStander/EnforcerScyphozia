...
namespace std {
  template<>
  struct hash<Enforcer::Model::Vertex> {
    size_t operator()(Enforcer::Model::Vertex const &vertex) const {
      size_t seed{0};
      Enforcer::HashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
      return seed;
    }
  };
} // namespace std
...
namespace Enforcer {
  ...
  */
  ...
  void Model::Data::LoadModel(const std::string &filepath) {
    ...
    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, (ROOT + filepath).c_str())) {
