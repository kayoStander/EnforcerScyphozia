...
namespace Enforcer {
  ...
  void RenderSystem::CreatePipeline(const VkRenderPass renderPass) {
    ...
    pipeline = std::make_unique<Pipeline>(device, "src/shaders/vertex.vert.glsl.spv",
  ...
  void RenderSystem::RenderGameObjects(const FrameInfo &frameInfo) const {
    ...
    for (u32 i{0}; i < frameInfo.gameObjects.size(); ++i) {
      ...
      vkCmdPushConstants(frameInfo.commandBuffer, pipelineLayout,
                         ...
                         &push);
      const float distanceFromCamera{glm::length(frameInfo.camera.getPosition() - obj.transform.translation)};
