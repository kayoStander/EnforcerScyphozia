...
void PointLightSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = std::make_unique<Pipeline>(
      device, "src/shaders/point_light.vert.glsl.spv",
      "src/shaders/point_light.frag.glsl.spv", pipelineConfig);
}
...
void PointLightSystem::Update(FrameInfo &frameInfo,
  ...
  for (auto &keyValue : frameInfo.gameObjects) {
    ...
    ASSERT_LOG(lightIndex < MAX_LIGHTS,
               "number of lights is bigger than the limit");

    obj.transform.translation =
        glm::vec3(rotateLight * glm::vec4(obj.transform.translation, 1.f));

    uniformBufferObject.pointLights[lightIndex].position =
        glm::vec4(obj.transform.translation, 1.f);
    uniformBufferObject.pointLights[lightIndex].color =
