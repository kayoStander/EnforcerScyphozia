...
namespace Enforcer {
  ...
  VkResult SwapChain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t const *imageIndex) {
  ...
  void SwapChain::createImageViews() {
    ...
    LOG_INFO(Vulkan, "Number of swapChainsImageViews => {}", swapChainImageViews.size());
  }
  ...
  VkPresentModeKHR SwapChain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes) {
    for (const auto &availablePresentMode: availablePresentModes) {
      if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
        LOG_INFO(Vulkan, "Present mode: Mailbox");
